...
Write-Host "Configuring Sample App Infra Requirements"  -ForegroundColor DarkGreen
# Setup a BrokerAuthentication component with an audience for the Sample App
kubectl apply -f $PSScriptRoot/yaml/broker-authn.yaml
...
kubectl get configmap aio-ca-trust-bundle-test-only -n azure-iot-operations -o yaml | % {$_.replace("namespace: azure-iot-operations","namespace: sample-app")} | kubectl apply -f - 
